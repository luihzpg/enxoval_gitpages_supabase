# Lista de Enxoval — GitHub Pages + Supabase

A simple enxoval list. Add items, delete items. No login, no framework, no build step.
...
2. Paste the contents of [`supabase-setup.sql`](supabase-setup.sql) and click **Run**. This creates the exclusive `enxoval_items` table.
...
git init
git add .
git commit -m "Initial enxoval list"
...
- inserts a temporary `__keepalive__...` row into `enxoval_items`
